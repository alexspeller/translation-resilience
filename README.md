...
Framework-agnostic (it patches the DOM layer, not React), dependency-free, ~4.3 kB min+gzip, and lazily activated — near-zero cost until a translator actually touches the page.
...
- **Firefox: install before translation starts.** Firefox's only signal is its `<html lang>` write at the start of a translation, and it leaves no marker behind, so a shim installed after that write (say, lazily loaded well after page load while "Always translate" is on) cannot tell the page is being translated. Installing in your entrypoint, as above, is early enough.
