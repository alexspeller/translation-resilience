...
- **The translator's own `<font>` wrappers.** Not every translator marks `<html>`: Microsoft Edge's built-in translator and the Google Translate browser extension wrap text in `<font>` elements without ever touching the class or `lang`. The patched `insertBefore`/`appendChild` recognise a `<font>` entering the tree — an element renderers never emit — and arm on it, in time to record that same displacement. While dormant this is one `nodeName` comparison per insert.
